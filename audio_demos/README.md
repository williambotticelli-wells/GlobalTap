...
consensus vs Reference annotation) using bit-identical DSP across
...
|-----------------------------|----------------------------|----------------------|---------------------------------------------------------------------|
| `candombe_phase_offset`     | `proyecto.1992_lobo_06`    | structural divergence | Phase offset (~140 ms; P-center-style perceived-onset shift, Morton 1976; Danielsen 2019). |
...
| `ballroom_baseline`         | `Albums-AnaBelen_Veneo-13` | high-consensus contrast | High-consensus baseline — CV ≈ 0.024; Crowd ≈ Reference.            |
...
- `<case-label>__playhead.mp4`  — Figure-2 layout + moving playhead;
  Crowd then Reference click overlay back-to-back (~30 s).
...
These case studies are discussed in paper §3.6; the bundled
...
# Convert to MP3 (matching the canonical filenames above):
...
output and the upstream reference annotation. The `madmom` and
