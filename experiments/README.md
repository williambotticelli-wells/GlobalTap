...
- Goldsmiths-MSI item text in `gmsi.py` is referenced from the
  experiment but only redistributed in the upstream PsyNet
...
- `data/`, `exports/` — the raw PsyNet exports, which carry platform
  identifiers and per-session metadata, are not redistributed here (see the
  paper's Ethics Statement). The de-identified benchmark taps from those exports
  are released in `../data/taps/`, and the Stage-2/3 snapshots behind the released
  consensus annotations live in `../data/stage_inputs/<dataset>/`.
- `.venv/` — Python virtual environment. Recreate from
  `requirements.txt`.

## Data flow back into the analysis pipeline

After a deployment, PsyNet exports a per-deployment `data/` folder.
To feed those into the paper's analysis pipeline:

1. Convert / merge into the canonical
