#!/usr/bin/env bash
# Drive the canonical pipeline end-to-end from a single config.
#
# Usage:
#   PROJECT_ROOT=/path/to/data ./run_pipeline.sh [config/canonical240.json] [stages]
#
# Examples:
#   ./run_pipeline.sh                              # all stages on canonical240
...
# those stages are skipped automatically; for the GlobalTap benchmark that
# leaves stages 1, 3 and 4, and its Stage-2 KDE peaks are rebuilt from the
# released taps with scripts/kde_peaks_from_taps.py instead.
#
...
if stage_has 6; then
  echo "[run_pipeline] --- Stage 6: period audit ---"
