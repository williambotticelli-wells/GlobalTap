...
def main():
    ...
    ap.add_argument(
        ...
        help="Project root used to resolve relative upstream-annotator paths in the GT map.",
    )
    ap.add_argument(
        "--mirex_gt_annotator",
        type=int,
    ...
    ap.add_argument("--skip_figures", action="store_true",
                    help="Skip per-stem overlay PNGs and summary figures (for sweeps).")
    ...
    for stem in sorted(by_stem.keys()):
        ...
        # sonify step (both only consume CPU; no downstream CSVs depend on them).
