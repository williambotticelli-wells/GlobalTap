...
the PsyNet exports. Cleaning, kernel density estimation, and peak
...
With ``--verify`` the recomputed peaks are compared against the bundled
``data/stage_inputs/<dataset>/cleaned_kde_peaks.csv`` snapshot.
"""
...
MIN_DISTANCE_S = 0.15
MAD_FACTOR = 3.5
PEAK_DECIMALS = 3  # the released snapshot stores peak times to the millisecond
...
def main() -> None:
    ...
    for stem, peaks in computed.items():
        ...
        if gap <= 0.001:
            ...
            differing += 1
    print(f"  peak times equal to the snapshot   : {identical}")
    print(f"  one 5 ms grid step from snapshot   : {same_count}")
