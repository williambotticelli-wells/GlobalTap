...
These are the full PsyNet exports and are not distributed in this public
companion repository (`data/taps/` holds the de-identified benchmark taps, and
`data/SOURCES.md` explains why); point `--merged-dir` at a tree
that mirrors `raw_tapping_data/merged/` to recompute `data/per_stem_metrics.
csv`'s `istc` column (verified: 240/240 stems, Spearman rho=0.99997 against the
released values, median absolute difference 0.03 and maximum 0.2 on the 10-90
scale -- a handful of per-second maxima differing by one participant, from
small trial-level filtering differences rather than rounding).
