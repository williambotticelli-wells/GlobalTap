...
Computes inter-subject tapping coherence (ISTC; Granot et al. 2023) per
(stem x tapping_country) cell with equal-N bootstrap subsampling, then
runs a stem- and tapping-country-fixed-effects model for the five balanced
source countries (US, KR, FR, MX, EG) plus all-10-country descriptive
summaries.
...
# ---------- per-cell bootstrap subsampling ----------
...
def cell_istc_subsampled(
    ...
    *,
    ...
    """Return (mean ISTC over n_boot subsamples of size m, n_participants).
...
def compute_per_cell(
    ...
    print(f"[istc] wrote {len(per_cell)} (stem,country) cells")
    return per_cell
...
def add_source_country(per_cell: pd.DataFrame, cfg: dict) -> pd.DataFrame:
    ...
    if n_missing > 0:
        print(f"[istc] WARNING: {n_missing} cells missing source_country")
    return merged
...
def write_summary(
    ...
    lines = [
        ...
        f"- **Subsampling**: {cfg['subsampling']['subsample_n']} tappers per (stem,country) cell; "
            f"{cfg['subsampling']['n_boot']} bootstrap draws; "
            f"cells with < {cfg['subsampling']['min_tappers_per_cell']} tappers dropped.",
        ...
        f"- Cells: {fe_model['n_cells']} across {fe_model['n_stems']} stems and "
            f"{fe_model['n_tapping_countries']} tapping countries; "
            f"{fe_model['n_source_matched_cells']} source-matched cells.",
    ...
    # Paper wording follows the nested fixed-effects model, not the
    # overlapping country-wise meta-analysis.
    fe_effect = fe_model["effect_istc_participants_per_bin"]
    fe_ci = fe_model["ci95"]
    fe_p = fe_model["p_two_sided"]
    n_pos = m_adj["n_countries_with_pos_effect"]
    k = meta["k_countries"]
    if fe_p < 0.05 and fe_effect > 0:
        verdict = "preliminary evidence for a country-matching effect after "\
                  "controlling for excerpt and country-panel baselines."
    else:
        verdict = "no evidence for a country-matching effect once country-level "\
                  "and excerpt-level differences in tapping coherence are controlled for."
    lines += [
        "## Claim for paper (do not overclaim)",
        "",
        "> We tested whether tapping coherence was higher for excerpts whose "
            "source country matched the participant recruitment country. "
            "For the five intentionally balanced source countries (US, KR, "
            "FR, MX, EG; 20 excerpts each), we estimated equal-N inter-subject "
            "tapping coherence for every excerpt-by-country-panel cell. A model "
            "with excerpt and tapping-country fixed effects, with standard "
            "errors clustered by excerpt, estimated a source/tapper-country "
            f"match effect of {fe_effect:+.2f} ISTC units "
            f"(95% CI [{fe_ci[0]:+.2f}, {fe_ci[1]:+.2f}], "
            f"two-sided p = {fe_p:.3f}). We therefore found {verdict} "
            f"A secondary country-wise sensitivity analysis showed positive "
            f"adjusted effects in {n_pos}/{k} balanced countries. "
            "AU, BR, JP, PL and ZA were each represented by only two "
            "source-country excerpts in the benchmark and are reported "
            "descriptively in the main-text figure only, not treated as "
            "independent country-level tests.",
        "",
    ]
