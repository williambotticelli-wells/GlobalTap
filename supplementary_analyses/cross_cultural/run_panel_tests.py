...
def main() -> None:
    ...
    lines = [
        ...
        "",
        f"- Trials: {len(trials):,}; participants: "
        f"{trials['participant_id'].nunique():,}; excerpts: "
        f"{trials['stim_name'].nunique()}.",
        "- Per-trial taps were cleaned by the canonical adjacent-IOI MAD filter.",
        "- Omnibus tests residualize outcomes by excerpt, aggregate to the "
        "participant level, and permute panel labels across participants.",
        "- These permutation tests condition on the sampled excerpts and "
        "leave-panel-out references; they do not propagate excerpt-sampling "
        "or reference-estimation uncertainty.",
        ...
        f"- Metrical-choice pairwise post hoc: {n_at_floor} of {n_pairs} panel "
        f"pairs tie at the FDR-corrected floor q={tactus_floor:.4g}; no single "
        "pair is resolvable as strongest. See tactus_pairwise_panel_tests.csv "
