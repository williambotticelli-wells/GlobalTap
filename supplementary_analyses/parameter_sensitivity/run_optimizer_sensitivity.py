...
def main() -> None:
    ...
    lines = [
        ...
        "",
        "Each row changes one active parameter around the canonical setting while "
        "holding the Stage-2 peaks and all other settings fixed.",
        "",
    ]
    for row in summary.itertuples(index=False):
        lines.append(
            f"- {row.variant}: F vs canonical={row.mean_f_vs_canonical:.3f}; "
            f"matched-rate vs canonical={row.matched_rate_vs_canonical:.3f}; "
            f"delta F vs expert references={row.mean_delta_f_vs_gt:+.3f}."
        )
