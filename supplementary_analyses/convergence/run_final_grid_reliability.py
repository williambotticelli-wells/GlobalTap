#!/usr/bin/env python3
"""Final-grid reliability using the exact canonical GlobalTap pipeline.

This analysis complements KDE/peak convergence by rerunning canonical Stage 2
...
def write_summary(
    ...
    comparison = pd.read_csv(out_dir / "final_746_vs_released_consensus.csv")
    lines = [
        "# Final consensus-grid reliability",
        "",
        "- Subsets are passed through the canonical per-trial MAD cleaning, "
        "5-ms KDE/peak extraction, and canonical Stage-4 grid optimizer.",
        "- The full 746-participant merge is used only for this audit; its "
        "de-identified taps are released in `data/taps/`.",
        "- At each N, one disjoint A/B split was evaluated per excerpt; the "
        "higher-repetition KDE/peak analysis provides the dense convergence curve.",
        f"- Final full-pool grids generated: {len(full)}.",
        f"- Mean F between final full-pool and previously released grids: "
        f"{comparison['f_measure'].mean():.3f}.",
        "",
        "## Subset results",
        "",
    ]
...
def main() -> None:
    ...
    parser.add_argument(
        "--n-grid", type=int, nargs="+", default=[5, 10, 20, 30, 50]
    )
