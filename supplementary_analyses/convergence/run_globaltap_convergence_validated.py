...
def main() -> None:
    ...
    axes[0].set_ylabel("Mean density correlation")
    ...
    lines = [
        ...
        "- Primary curve compares random N-tapper consensuses with the full available "
        "consensus; an independent N-vs-N curve checks overlap optimism.",
        "- Confidence intervals bootstrap stems, not repeated subsamples.",
        f"- Smallest tested N with mean peak F-measure >= .90: {knee_f90}.",
        f"- Smallest tested N with mean density correlation >= .90: {knee_r90}.",
        "",
        "These thresholds describe this corpus, preprocessing, and metric; they are "
        "not universal power guarantees.",
    ]
