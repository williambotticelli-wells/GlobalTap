...
- The tapping-derived GlobalTap consensus is the `mir_eval` reference and each tracker's beat sequence is the estimate. Raw individual taps are not scored.
- GlobalTap consensus stems: 200.
- Paired algorithm comparisons: 198 stems; 2 excluded because Beat This! returned fewer than two beats in the evaluation window.
- Metrics computed with `mir_eval` 0.8.2.
- Paired two-sided Wilcoxon tests compare algorithms stem by stem; BH-FDR is applied across all metric comparisons.
