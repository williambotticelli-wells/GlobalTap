...
def main() -> None:
    ...
    lines = [
        ...
        "and each tracker's beat sequence is the estimate; raw individual taps "
        "are not scored.",
        f"- GlobalTap consensus stems: {len(references)}.",
        f"- Paired algorithm comparisons: {len(paired_stems)} stems; "
        f"{len(exclusions)} excluded because Beat This! returned fewer than two "
