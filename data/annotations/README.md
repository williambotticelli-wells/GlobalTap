...
click-track listening stimuli were rendered from. File contents are identical to
the pipeline's `crowd_gs` output; only the extension differs.
...
Step 1 returns the bundled `data/stage_inputs/globalmood200/cleaned_kde_peaks.csv`
snapshot, matching it excerpt for excerpt within the 5 ms analysis grid. Step 2
turns those peaks into the grids in `globalmood200/` here: run in a fresh clone it
reproduces all 200 files, with the largest deviation across every beat below a
microsecond. The files here are rounded to nine decimals; Stage 4 writes full
precision.
...
Licensed CC BY 4.0. The source audio is not redistributed; see
