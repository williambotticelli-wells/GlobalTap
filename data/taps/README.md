...
For the validation set, the released artifacts are the Stage-2 peaks in
[`../stage_inputs/canonical240/`](../stage_inputs/canonical240/) and the
consensus annotations in [`../annotations/canonical240/`](../annotations/canonical240/),
which are the grids the click-track rating stimuli were rendered from.
...
This returns the bundled
`data/stage_inputs/globalmood200/cleaned_kde_peaks.csv` snapshot, matching it
excerpt for excerpt within the 5 ms analysis grid. Stage 4 then turns those peaks
back into the released consensus annotations
([`../annotations/`](../annotations/)).

## Provenance

`scripts/build_tap_release.py` builds these tables from the PsyNet exports,
which are not redistributed. Taps are parsed with
