...
Crowdsourced Tapping."** This repository hosts the canonical pipeline, the
de-identified tap data, and scripts supporting the paper's numerical claims and
figures. A few analyses that need the raw audio provide code and aggregate
outputs but cannot be rerun from the public tree alone. A
self-contained HTML
...
variable (or by editing the dataset config) — see §2. Tap data is released
under pseudonymous participant labels; no platform identifiers, timestamps, or
participant-level demographics are distributed.
...
| Listener ratings (validation) | 6,487 (median 15 per stem–condition cell) |
...
│   ├── 1_merge_exports.py         # Merge tap CSVs; analysis_failed=False filter
...
│   ├── 4_run_crowd_gs.py          # Crowd grid-search optimizer (canonical)
│   ├── 5_compare_variants.py      # F-measure + offset comparisons
│   ├── 6_period_audit.py          # Reference vs crowd period audit
...
│   ├── manifest.py                # Run-provenance manifests
│   ├── manifest_cli.py
│   └── paths.py                   # Config loader + ${PROJECT_ROOT} expansion
│
├── optimization/              # Beat-grid optimizer (auto-detection cascade)
│   ├── run_pipeline.py            # Cascade entry point: run_cascade(...)
│   ├── optimization_regularize.py # Grid-search optimizer + post-processing
│   └── detect_multimodal.py       # Bimodal-phase diagnostic
│
├── scripts/                   # Modules the stages call out to
│   ├── repp_gt_alignment_pipeline.py   # REPP audio-onset alignment
│   ├── build_tap_release.py       # Builds data/taps/ from the PsyNet exports
│   ├── kde_peaks_from_taps.py     # Stage 2 from released taps (verifies the snapshot)
...
│   ├── stage_inputs/                              # Stage-1/2/3 snapshots from the validation runs
...
│   └── SOURCES.md                                 # Where to download upstream audio + annotations
│
...
│   ├── metrical_preference/        # Complete 180-excerpt rating audit
│   ├── parameter_sensitivity/      # 240-excerpt optimizer-setting sensitivity audit
│   └── METHOD_PARAMETERS.md        # Fixed settings and concise rationales
│
...
figure scripts read only from `data/` and from the bundled
...
The repo bundles aggregate Stage-2/3 outputs from the validation runs in
...
with no `PROJECT_ROOT` and no audio
(Stage 4 transparently falls back to the bundled inputs when no full
pipeline tree is configured). This exactly reproduces the `cv_pipeline`
...
This returns the bundled Stage-2 snapshot, matching it excerpt for excerpt
...
The run configuration records canonical values under `alignment.*` and
`optimizer.*`. Several Stage-4 micro-parameters are still supplied by matching
function defaults rather than dynamically read from the config. The heuristic
settings were selected during pipeline development using diagnostic checks and
sensitivity analyses, then held fixed across the reported excerpts and
corpora. A bounded 240-excerpt check passed active-setting overrides directly
to the optimizer and retained final-grid F=.947--1.000, with the tactus
unchanged in 92.1--100% of cases. See
`supplementary_analyses/METHOD_PARAMETERS.md` for their effective values and
rationales.
...
algorithm participates in Stage 4 — the canonical `crowd_gs` consensus
...
The optimizer's two-term grid score
**S(T, φ) = D̄(b) + 0.5 · C(b, p)** and the bimodal diagnostic
(largest sub-period IPI gap > 0.10·T) live in
`optimization/optimization_regularize.py` and
`optimization/detect_multimodal.py`. See **`supplementary.html`** for
the concise parameter-rationale record.

### `clip_offset_s` in `gt_source_map_*.json`

Source corpora time their beats against the full-length track, but
the GlobalTap pipeline operates on 30 s clips. `clip_offset_s` is the
clip's start time in seconds; `lib/gt_loader.py` subtracts it so all
beat times are reported clip-local (`t_clip = t_annotator −
clip_offset_s`). Each entry also carries a `row_filter` (e.g.
`"pos>=1"` to drop unlabeled annotator rows) and a `source_label`
identifying the source-corpus annotation pack.

---

## 4. Reproducing the paper figures

The bundled `data/` directory ships with the per-stem aggregates that
...
To re-derive `data/per_stem_metrics.csv` from raw inputs (and reproduce
...
The pipeline is deterministic given the config and pinned deps;
rerunning on the same inputs produces bit-identical beat sequences and
per-stem metrics.
...
- **Audio**: GlobalTap benchmark source recordings are not redistributed. The
...
During original pipeline development, large-language-model tools were used
to help conduct the diagnostic sensitivity analyses that informed the fixed
heuristic parameters described in this README and in
`supplementary_analyses/METHOD_PARAMETERS.md`. The authors selected and
verified the final values. During final revisions, large-language-model
tools additionally helped refine the design of the supplementary analyses
added at that stage, check their implementation, and draft parts of this
documentation. The authors verified all code and outputs and remain
responsible for this repository. See also the AI Usage Statement in the
paper.
