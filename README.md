...
To rebuild the consensus beat grids and the click-track WAVs used in the
listening experiment, run the full pipeline (stages 1–7) over
`${PROJECT_ROOT}` as shown in §2. Stages 1–7 do not aggregate listener
ratings, so they do not regenerate `data/per_stem_metrics.csv`, whose rating
columns come from the validation study. That path is
...
Python 3.11+ for the core stack: the pinned `numpy`, `pandas` and `scipy`
versions require it. Key libraries: `numpy`, `pandas`, `scipy`, `matplotlib`,
`librosa`, `madmom`. See `requirements.txt` for pinned versions.
Madmom requires CPython ≤ 3.11, so 3.11 satisfies both constraints.
