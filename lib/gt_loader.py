...
returns clip-local times + position labels for a given 30-s window.
("gt" in file/variable names below is a historical label for these
source-corpus reference annotations, not a claim that they are ground
truth in an absolute sense -- see the paper's discussion of listener-
derived vs. structurally informed references.)

Designed to be dataset-agnostic: the same loader serves canonical240,
globalmood200 (where a reference annotation is null -> returns None), and
any future corpus that supplies a gt_source_map.
