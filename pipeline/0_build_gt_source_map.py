...
`next-steps/benchmark100-validation/ANNOTATION_SOURCES.md`):
    ...
    rwc_classical_CD1_01   : path=audio/benchmark_30s/... , row_filter=pos>=1
                             (bm30_filt; bit-identical to upstream in
                             the 30-s window; beat_this upstream has a
...
CD1_01_OVERRIDE = {
    ...
    "row_filter": "pos>=1",
    "note": (
        "bm30_filt (drop conversion-bug pos==0 rows). Verified "
        "bit-identical to beat_this upstream in the 30-s window "
        "(offset 5.87s). Full-track upstream has a two-regime "
        "conversion bug flagged in the beat_this README."
    ),
}
