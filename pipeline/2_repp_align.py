...
def main():
    ...
    ap.add_argument(
        ...
        help="Skip per-stem overlay PNGs (speeds up sweeps).",
    )
    ap.add_argument(
        "--out_subdir", default="repp",
        help="Subdir under outputs_root to write to (default: 'repp'). Use to stage sweep cells.",
    )
    # Optional CLI overrides for sweep cells — None means 'use config value'.
