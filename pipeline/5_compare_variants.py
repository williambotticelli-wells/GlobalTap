...
def main():
    ...
    if "${" in os.path.expandvars(data_root_str):
        ...
        # clone can run Stage 5 against the bundled stage_inputs/.
        data_root = HERE
    ...
    # Fall back to the bundled stage-3 outputs that ship with the repo so
    ...
    if not madmom_csv.exists() and (bundled / "algorithm_beats.csv").exists():
        madmom_csv = bundled / "algorithm_beats.csv"
    if not beat_this_csv.exists() and (bundled / "beat_this_algorithm_beats.csv").exists():
        beat_this_csv = bundled / "beat_this_algorithm_beats.csv"
