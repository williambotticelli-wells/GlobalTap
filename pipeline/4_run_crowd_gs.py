...
def main():
    ...
    if "${" in data_root_str:
        ...
        # clone can run Stage 4 against the bundled stage_inputs/ snapshot.
        data_root = HERE
    ...
    # ``out_root`` use it; otherwise fall back to the bundled
    # ``data/stage_inputs/<dataset>/`` snapshot that ships with the repo
    # so Stage 4 can run without raw audio or tapping CSVs.
    bundled = HERE / "data" / "stage_inputs" / cfg["dataset"]
    peaks_csv = (
    ...
    if not peaks_csv.exists() and (bundled / "cleaned_kde_peaks.csv").exists():
        peaks_csv = bundled / "cleaned_kde_peaks.csv"
    if not algo_csv.exists() and (bundled / "algorithm_beats.csv").exists():
        algo_csv = bundled / "algorithm_beats.csv"
